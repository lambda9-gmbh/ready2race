...
data class DataCompetitionCategoriesExport(
    ...
    companion object {
        ...
        fun importData(data: DataCompetitionCategoriesExport): App<WebDAVError.WebDAVImportNextError, Unit> = KIO.comprehension {
            try {
                val overlaps = !CompetitionCategoryRepo.getOverlapIds(data.competitionCategories.map { it.id }).orDie()
