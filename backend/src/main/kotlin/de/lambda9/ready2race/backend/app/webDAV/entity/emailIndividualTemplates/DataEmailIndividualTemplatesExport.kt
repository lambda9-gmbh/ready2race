...
data class DataEmailIndividualTemplatesExport(
    ...
    companion object {
        fun createExportFile(
            ...
            val json = !WebDAVExportService.serializeDataExportNew(record, mapOf("emailIndividualTemplates" to emailIndividualTemplates))

            KIO.ok(File(name = getWebDavDataJsonFileName(WebDAVExportType.DB_EMAIL_INDIVIDUAL_TEMPLATES), bytes = json))
        }

        fun importData(data: DataEmailIndividualTemplatesExport): App<WebDAVError.WebDAVImportNextError, Unit> = KIO.comprehension {
