...
data class DataStartlistExportConfigsExport(
    ...
    companion object {
        fun createExportFile(
            ...
            val json = !WebDAVExportService.serializeDataExportNew(record, mapOf("startlistExportConfigs" to startlistExportConfigs))

            KIO.ok(File(name = getWebDavDataJsonFileName(WebDAVExportType.DB_STARTLIST_EXPORT_CONFIGS), bytes = json))
        }

        fun importData(data: DataStartlistExportConfigsExport): App<WebDAVError.WebDAVImportNextError, Unit> = KIO.comprehension {
