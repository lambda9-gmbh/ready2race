...
const calcAmountForCompetitionRegistration = (
    ...
    competition.fees
        ?.filter(fee => fee.required || optionalFees.includes(fee.id))
        .reduce(
            (sum, fee) => sum + Number(late ? (fee.lateAmount ?? fee.amount) : fee.amount),
            0,
        ) ?? 0
    ...
    participants.reduce(
        (sum, participant) =>
            ...
            (participant.competitionsSingle
                ?.filter(reg => reg.isLate === late)
                .reduce((sumP, reg) => {
                    const competition = competitionMap.get(reg.competitionId)
                    return (
                        sumP +
                        (competition
                            ? calcAmountForCompetitionRegistration(
                                  competition,
                                  reg.optionalFees ?? [],
                                  late,
                              )
...
const calcAmountForTeams = (
    ...
    competitionRegistrations.reduce(
        (sum, comp) =>
            sum +
            comp.teams
                .filter(team => team.isLate === late)
                .reduce((sumT, team) => {
...
export const EventRegistrationFeeDisplay = () => {
    ...
    const competitionMap = useMemo(() => {
        return new Map(
            [...(info?.competitionsSingle ?? []), ...(info?.competitionsTeam ?? [])].map(
                competition => {
    ...
    const teamAmount = useMemo(
        () => calcAmountForTeams(competitionRegistrations, false, competitionMap),
        [competitionRegistrations],
    )
    const singleAmount = useMemo(
        () => calcAmountForSingles(participants, false, competitionMap),
        [participants],
    )
    const lateTeamAmount = useMemo(
        () => calcAmountForTeams(competitionRegistrations, true, competitionMap),
        [competitionRegistrations],
    )
    const lateSingleAmount = useMemo(
        () => calcAmountForSingles(participants, true, competitionMap),
        [participants],
    )
                ...
                {info?.state !== 'REGULAR' && (
                    <>
                        <Typography variant={'caption'}>
                            {t('event.registration.lateTotalFees')}
                        </Typography>
                        <Typography variant={'h5'}>
                            {(lateTeamAmount + lateSingleAmount).toFixed(2)}€
                        </Typography>
                    </>
                )}
            </Stack>
